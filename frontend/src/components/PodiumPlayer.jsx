const PodiumPlayer = ({ position, username, points, id }) => {
  ...
  return (
    <div className={`relative grow flex flex-col uppercase w-fit p-4 md:px-8 text-xl md:text-2xl items-center justify-center bg-white shadow-buttons border-3 border-neroNonNero font-bold font-primary ${transformClasses[position] || ""}`}>
      ...
      {position == 1 && (
        <span className="absolute -top-5 -right-10 rotate-10 text-[10px] md:text-sm font-bold bg-secondary py-2 px-3 shadow-buttons border-3 border-neroNonNero text-black">
          Vincitore
        </span>
      )}
      <img
      ...
      <span className="text-terziary text-lg md:text-xl"> {points} PTS </span>
    </div>
  );
};
