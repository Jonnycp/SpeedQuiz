...
async function getProfileStats(req, res) {
  try {
    ...
    //* cerco l'utente loggato dentro l'arrey dei giocatori della partita
    const formattedGames = recentGames.map((game) => {
      ...
      return {
        id: game._id,
        title: `STANZA DI ${game.hostUsername ? game.hostUsername.toUpperCase() : "SCONOSCIUTO"}`,
    ...
    return res.status(500).json({ message: "Impossibile caricare il profilo in questo momento." });
  }
}
